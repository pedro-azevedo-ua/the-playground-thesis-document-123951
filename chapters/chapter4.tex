\chapter{Phase 1: Prototype and Feature Validation}
	...
	\section{Produce design solutions to meet user requirements}
	
	With user requirements, the prototypes can be build.
	For this it was used Figma \footcite{https://www.figma.com/}, a design tool to build interactive high fidelity prototypes..
...
To allow users to find courts efficiently, the "Courts List" screen was designed with a dual-view approach. Users can toggle between a standard list for quick scanning and an interactive map to visualize court locations spatially. Key features include:
\begin{itemize}
	\item \textbf{Filtering and Sorting:} Options to organize courts by proximity, rating, or popularity.
	\item \textbf{Favorites:} A quick-access button to save frequently visited courts.
	\item \textbf{Community Contribution:} A "plus" button near the search bar allows users to submit new courts, ensuring the database remains up-to-date through crowdsourcing.
\end{itemize}

\subsubsection{Court Details and Functional Hub}
Once a user selects a court, they are taken to the "Court Details" screen. Because of the high volume of features required (checking games, reviews, and rankings), a tabbed navigation system was chosen for this screen. This design allows the user to access complex data without leaving the context of the specific court. As shown in Figure \ref{fig:nav-diag-courts}, the details are organized into five functional tabs:
...
\begin{itemize}
	...
	\item \textbf{Rank Tab:} Displays the local leaderboard for that specific court.
...
The navigation flow, represented in Figure \ref{fig:nav-diag-courts}, follows a clear three-tier hierarchy to keep the interface simple. 
Users move from the global navigation bar to the Courts List, and finally into the Court Details . 

By using internal tabs within the Details screen, the design maintains a "shallow" navigation stack. This ensures that users can switch between checking the calendar and viewing the players list with a single tap, reducing the number of "back" actions required to navigate the module.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\textwidth]{figs/navigation-diagram/courts.png}
	\caption{Navigation Diagram - Courts flow}
